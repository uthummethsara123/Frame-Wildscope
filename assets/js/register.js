// ==========================================================================
// WILDSCOPE 2026 PARTICIPATION SYSTEM WITH GOOGLE IDENTITY & BACKEND HOOK
// ==========================================================================
...
const OPENING_DATE = new Date(2026, 6, 12, 0, 0, 0).getTime();      // July 12th, 2026
...
if(regForm) {
    regForm.addEventListener('submit', function(e) {
        ...
        // 🔒 1. MANDATORY SECURITY GATE: BLOCK UNAUTHENTICATED SUBMISSIONS
        ...
        // Fill remaining delegate slots up to 8 if unused
        for (let i = 2; i <= 8; i++) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- DYNAMIC DELEGATES ROSTER ADDITION (DARK MARATHON THEME) ---
    if (addDelegateBtn && delegateContainer) {
        addDelegateBtn.addEventListener('click', function(e) {
            ...
            delegateWrapper.className = 'delegate-form-block mb-4 p-3 border border-secondary rounded bg-dark bg-opacity-50 position-relative';
            delegateWrapper.id = `delegate-block-${window.globalDelegateCount}`;
            
            delegateWrapper.innerHTML = `
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h6 class="text-success fw-bold mb-0">Nominated Delegate #${window.globalDelegateCount}</h6>
                ...
                <div class="mb-3">
                    <label class="form-label text-light small">Full Name <span class="text-danger">*</span></label>
                    <input type="text" name="delegate_name_${window.globalDelegateCount}" class="form-control" placeholder="e.g. Kasun Perera" required>
                </div>
                <div class="mb-3">
                    <label class="form-label text-light small">Date of Birth (YYYY.MM.DD) <span class="text-danger">*</span></label>
                    <input type="text" name="delegate_dob_${window.globalDelegateCount}" class="form-control" placeholder="e.g. 2008.05.12" required>
                </div>
                <div class="mb-3">
                    <label class="form-label text-light small">WhatsApp Number <span class="text-danger">*</span></label>
                    <input type="tel" name="delegate_phone_${window.globalDelegateCount}" class="form-control" placeholder="07X XXX XXXX" required>
                </div>
                <div class="mb-3">
                    <label class="form-label text-light small">Delegation Dietary Option <span class="text-danger">*</span></label>
                    <select name="delegate_diet_${window.globalDelegateCount}" class="form-select bg-dark text-white border-secondary" required>
    ...
    function renumberDelegates() {
        ...
        blocks.forEach((block) => {
            ...
            block.querySelector('h6').textContent = `Nominated Delegate #${window.globalDelegateCount}`;
...
// ==========================================================================
function cleanUpAndDestroyPreloader() {
...
window.addEventListener("load", () => {
    setTimeout(cleanUpAndDestroyPreloader, 800);
});